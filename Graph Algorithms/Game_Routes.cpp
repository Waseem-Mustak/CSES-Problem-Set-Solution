...
ll  ara[N],parent[N],visited[N],path_cnt[N];
vector<ll>v[N];
//vector<ll>v(N),inv(N);
//ll arr[1001][1001];
...
void solve()
{
    ...
    // this prob can solve by topo sort (inner vertex method)
    ...
    for(auto x:topo_vec)
    {
        for(auto y:v[x])
        {
            path_cnt[y]=(path_cnt[y]+path_cnt[x])%M;
        }
    }
    cout<<path_cnt[n]<<el;
...
// ll  ara[N],ara1[N],ara2[N],Count[N],inner[N];
// vector<ll>v[N];
// //vector<ll>v(N),inv(N);
// //ll arr[1001][1001];
