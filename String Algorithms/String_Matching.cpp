...
// If we are searching for string P inside of string T, 
// create a new string S = P + # + T, where # is any 
// arbitrary character that does not occur in either string. 
// Then, create an array \pi_S using the KMP algorithm. 
// The answer is simply the number of indicies inside of 
// \pi_S that is equal to |P| (the length of P).
...
void solve()
{
    ...
    s3=s2+'#'+s1;
    vector<ll> tem=lps_array_of_kmp_algo(s3);
    ans=0;
    for(auto va:tem)
    {
        if(va==n)ans++;
