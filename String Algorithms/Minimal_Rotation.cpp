...
int minimumExpression(string s) {   // found this algo in anik sarker's vai template https://github.com/AnikSarker/ACM-Library/blob/master/String/Minimum%20Lexicographical%20Rotation.cpp
    s = s + s; 
...
void solve()
{
    ...
    string s1;
    cin>>s1;
    x=minimumExpression(s1);
    n=s1.size();
    string ans_st;
    lp(i,x,n-1)ans_st.push_back(s1[i]);
    lp(i,0,x-1)ans_st.push_back(s1[i]);
    cout<<ans_st<<el;
}
