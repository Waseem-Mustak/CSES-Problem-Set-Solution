...
void solve()
{
    ...
    cin>>k;
    a=9;
    ll number_of_digit=1;
    while(k>(a*number_of_digit))
    {
        k-=(a*number_of_digit);
        a*=10;
        number_of_digit++;
    }
    m=ara[number_of_digit]+((k-1)/number_of_digit)+1-1;
    if(k%number_of_digit==0)k=number_of_digit;
    else k%=number_of_digit;
 
    ll pos=number_of_digit-k;
...
// the digit we are searching ,its a part of a number(m),first find how many digit in this number,
// then try to find this number
// and minimize k
// after all of that,,try to find the digit 
// if we can find number m
// we can find the digit too,from remaining k
