...
vector<pair<pair<ll,ll>,ll>>inputs;
vec v1;
...
void solve()
{
    ...
    sort(inputs.begin(),inputs.end(),myComparison);
    srtv(v1);

    lp(i,1,n)
    {
        a=lower_bound(v1.begin(),v1.end(),inputs[i-1].f.f)-v1.begin();
        dp[i]=max(dp[i-1],dp[a]+inputs[i-1].s);
    }    
